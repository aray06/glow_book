        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="hidden md:flex items-center space-x-6">
                    <a href="{{ route('home') }}" class="text-stone-300 hover:text-gold transition text-sm font-medium">Home</a>
                    <a href="{{ route('salons') }}" class="text-stone-300 hover:text-gold transition text-sm font-medium">Salons</a>
                    <a href="{{ route('services') }}" class="text-stone-300 hover:text-gold transition text-sm font-medium">Services</a>
                    <a href="{{ route('specialists') }}" class="text-stone-300 hover:text-gold transition text-sm font-medium">Specialists</a>

                    @auth
                        @if(auth()->user()->isClient())
                            <a href="{{ route('client.dashboard') }}" class="text-stone-300 hover:text-gold transition text-sm font-medium">Dashboard</a>
                        ...
                        @elseif(auth()->user()->isSpecialist())
                            <a href="{{ route('specialist.dashboard') }}" class="text-stone-300 hover:text-gold transition text-sm font-medium">My Schedule</a>
                        @elseif(auth()->user()->isSalonOwner())
                            <a href="{{ route('owner.dashboard') }}" class="text-stone-300 hover:text-gold transition text-sm font-medium">My Salon</a>
                        @elseif(auth()->user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="text-stone-300 hover:text-gold transition text-sm font-medium">Admin</a>
                        @endif
                ...
                <div class="md:hidden" x-data="{ mobileOpen: false }">
                    ...
                    <div x-show="mobileOpen" @click.away="mobileOpen = false" x-transition class="absolute top-16 left-0 right-0 bg-brown border-t border-stone-700 shadow-xl p-4 space-y-3">
                        <a href="{{ route('home') }}" class="block text-stone-300 hover:text-gold transition py-1">Home</a>
                        <a href="{{ route('salons') }}" class="block text-stone-300 hover:text-gold transition py-1">Salons</a>
                        <a href="{{ route('services') }}" class="block text-stone-300 hover:text-gold transition py-1">Services</a>
                        <a href="{{ route('specialists') }}" class="block text-stone-300 hover:text-gold transition py-1">Specialists</a>
                        @auth
                            @if(auth()->user()->isClient())
                                <a href="{{ route('client.dashboard') }}" class="block text-stone-300 hover:text-gold py-1">Dashboard</a>
                                <a href="{{ route('client.book') }}" class="block text-gold font-semibold py-1">Book Now</a>
                            @elseif(auth()->user()->isSpecialist())
                                <a href="{{ route('specialist.dashboard') }}" class="block text-stone-300 hover:text-gold py-1">My Schedule</a>
                            @elseif(auth()->user()->isSalonOwner())
                                <a href="{{ route('owner.dashboard') }}" class="block text-stone-300 hover:text-gold py-1">My Salon</a>
                            @elseif(auth()->user()->isAdmin())
                                <a href="{{ route('admin.dashboard') }}" class="block text-stone-300 hover:text-gold py-1">Admin</a>
                            @endif
                        ...
                        @else
                            <a href="{{ route('login') }}" class="block text-stone-300 hover:text-gold py-1">Sign In</a>
